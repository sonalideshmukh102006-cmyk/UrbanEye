...
import ODAnalysisModal from './ODAnalysisModal';
...
export default function SidebarLeft() {
  ...
  const CorridorCard = ({ corridor }: { corridor: any }) => (
    <div
      ...
      className="bg-[#F8FBFE] rounded-xl p-3 border border-blue-100 hover:border-blue-300 transition-colors group cursor-pointer shrink-0 shadow-sm"
    >
      <div className="flex justify-between items-start mb-3">
        <div>
          <h3 className="text-[15px] font-bold text-blue-600">{corridor.name}</h3>
          <p className="text-xs text-gray-700 mt-1">{corridor.status} • Delay: {corridor.delay} • Source: {corridor.sourceBus} ({corridor.lastUpdatedMinutesAgo}m ago)</p>
        </div>
        <ChevronRight className="w-4 h-4 text-gray-800" strokeWidth={2.5} />
      </div>

      <div className="flex justify-between items-center mb-1.5">
        <span className="text-[11px] font-bold uppercase tracking-wider text-gray-800">Vehicle Density</span>
        <span className={`text-sm font-bold ${corridor.density > 75 ? 'text-red-500' : corridor.density > 50 ? 'text-orange-500' : 'text-green-500'}`}>
          {corridor.density}%
        </span>
      </div>
      ...
      <div className="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden mb-1">
        <div
          className={`h-1.5 rounded-full ${corridor.density > 75 ? 'bg-red-500' : corridor.density > 50 ? 'bg-orange-500' : 'bg-green-500'}`}
  ...
  const HotspotCard = ({ hotspot }: { hotspot: any }) => (
    <div key={hotspot.id} className="bg-background rounded-xl p-3 border border-border hover:border-purple-500/50 transition-colors group cursor-pointer flex flex-col gap-2 relative overflow-hidden shrink-0">
      ...
      <div className="flex justify-between items-center z-10">
        ...
        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${hotspot.density > 85 ? 'bg-red-500/10 text-red-500' :
  ...
  return (
    <div className="w-full h-full bg-[#FFF9F2] border border-gray-200 flex flex-col shrink-0 shadow-sm">
      ...
      <div className="p-4 bg-red-50/50 border-b border-red-100 shrink-0">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-sm font-bold text-red-600 uppercase tracking-widest flex items-center gap-2">
            <AlertTriangle className="w-4 h-4" /> Vechicle Bottleneck Points
          </h2>
          <span className="text-[9px] text-red-400 font-medium max-w-[120px] text-right leading-tight border border-red-200 rounded p-1 bg-white/50 shadow-sm" title="Estimated from live vehicle density × avg speed on corridor">
            *Delays dynamically estimated
          </span>
        </div>
          ...
          {MOCK_TRAFFIC_CORRIDORS
            ...
            .map(c => (
              <div
                ...
                className="bg-white rounded-lg p-3 border border-red-200 shadow-sm flex items-center justify-between hover:border-red-400 transition-colors cursor-pointer"
              >
                <div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between mb-1">
            ...
            <button 
              ...
              className="text-[10px] font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded flex items-center gap-1 transition-colors border border-blue-200"
            >
              <Map className="w-3 h-3" /> O-D Analysis
            </button>
          </div>
      ...
      {/* OD Analysis Modal */}
      {showODModal && <ODAnalysisModal onClose={() => setShowODModal(false)} />}
    </div>
  );
}
